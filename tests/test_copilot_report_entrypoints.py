...
class CopilotReportEntrypointTests(unittest.TestCase):
    ...
    def test_flow_report_helper_help_and_direct_entrypoint(self) -> None:
    ...
    def test_flow_report_works_without_active_workflow(self) -> None:
        scripts = self.repo_root / "skills/copilot/flow/scripts"
    ...
    def test_flow_report_script_is_executable_and_power_shell_is_thin(self) -> None:
        script = self.repo_root / "skills/copilot/flow/scripts/flow-report"
        self.assertTrue(os.access(script, os.X_OK))
        ps1 = (script.parent / "flow-report.ps1").read_text(encoding="utf-8")
        self.assertIn('invoke-flow.ps1', ps1)
        self.assertIn('flow-report.ps1', ps1)
        self.assertNotIn("copilot_report", ps1)
        self.assertNotIn("render_latest_copilot_report", ps1)
