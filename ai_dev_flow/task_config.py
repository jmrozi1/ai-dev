...
def default_user_config_text() -> str:
    return (
        ...
        "  aliases:\n"
        "    # false: keep definitions in config but do not install/reconcile managed aliases.\n"
        "    # true: reconcile managed aliases from commands.\n"
        "    enabled: true\n"
        "    # Global policy for generating prefixed direct-subcommand launchers from each root alias.\n"
        "    # false: install only the aliases listed under commands.\n"
        "    # true: include generated direct subcommand descendants when an authoritative model exists.\n"
        "    expand_subcommands: true\n"
        "    commands:\n"
        "      flow: \"ai-dev flow\"\n"
        "      # Advanced exact-token form is also accepted and normalized internally:\n"
        "      # array tokens are preserved verbatim (including intentional surrounding spaces).\n"
        "      # example:\n"
        "      #   - ai-dev\n"
        "      #   - command\n"
        "      #   - \"one argument\"\n"
