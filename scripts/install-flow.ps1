...
function Show-Help {
    @'
Usage: scripts/install-flow.ps1 [bootstrap-options]
...
Common examples:
  .\scripts\install-flow.ps1
  .\scripts\install-flow.ps1 --home $HOME --install-dir "$HOME\.local\bin"
...
$pythonExecutable = Resolve-AiDevPythonExecutable -CallerName 'install-flow.ps1'
...
try {
    ...
    [Console]::Error.WriteLine("install-flow.ps1: $($_.Exception.Message)")
    exit 1
}
finally {
